...
public class GuiAccountSelector extends Screen {
	...
	protected void init() {
		...
		addButton(new Button(this.width / 2 + 4 + 40, this.height - 52, 120, 20, I18n.format("ias.addaccount"), btn -> add())); //0
		addButton(login = new Button(this.width / 2 - 154 - 10, this.height - 52, 120, 20, I18n.format("ias.login"), btn -> login(selectedAccountIndex))); //1
		addButton(edit = new Button(this.width / 2 - 40, this.height - 52, 80, 20, I18n.format("ias.edit"), btn -> edit())); //7
		addButton(loginoffline = new Button(this.width / 2 - 154 - 10, this.height - 28, 110, 20, I18n.format("ias.login") + " " + I18n.format("ias.offline"), btn -> logino(selectedAccountIndex))); //2
		addButton(new Button(this.width / 2 + 4 + 50, this.height - 28, 110, 20, I18n.format("gui.cancel"), btn -> escape())); //3
		addButton(delete = new Button(this.width / 2 - 50, this.height - 28, 100, 20, I18n.format("ias.delete"), btn -> delete())); //4
	...
	public void render(int mx, int my, float delta) {
		...
		this.drawCenteredString(font, this.title.getFormattedText(), this.width / 2, 4, -1);
		if (loginfailed != null) {
		...
		if (!queriedaccounts.isEmpty()) {
			...
			if (queriedaccounts.get(selectedAccountIndex).premium != null) {
				if (queriedaccounts.get(selectedAccountIndex).premium) this.drawString(font, I18n.format("ias.premium"), width - 8 - 61, height / 2 - 64 - 13, 6618980);
				else this.drawString(font, I18n.format("ias.notpremium"), width - 8 - 61, height / 2 - 64 - 13, 16737380);
			}
			this.drawString(font, I18n.format("ias.timesused"), width - 8 - 61, height / 2 - 64 - 15 + 12, -1);
			this.drawString(font, String.valueOf(queriedaccounts.get(selectedAccountIndex).useCount), width - 8 - 61, height / 2 - 64 - 15 + 21, -1);
			if (queriedaccounts.get(selectedAccountIndex).useCount > 0) {
				this.drawString(font, I18n.format("ias.lastused"), width - 8 - 61, height / 2 - 64 - 15 + 30, -1);
	 ...
	 */
	private void add() {
		minecraft.displayGuiScreen(new GuiAddAccount());
	}
	 ...
	 */
	private void edit() {
		minecraft.displayGuiScreen(new GuiEditAccount(selectedAccountIndex));
	}
