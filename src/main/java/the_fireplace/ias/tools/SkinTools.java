...
public class SkinTools {
	...
	public static void cacheSkins(boolean ingame) {
		...
		for (int i = 0; i < AltDatabase.getInstance().getAlts().size(); i++) {
			...
			if (!ingame) GLFW.glfwSetWindowTitle(Minecraft.getInstance().mainWindow.getHandle(), "IAS: Updating skin " + data.alias + "...");
			...
			if (!ingame) GLFW.glfwSetWindowTitle(Minecraft.getInstance().mainWindow.getHandle(), "Minecraft 1.13.2"); 
		}
	}
}
