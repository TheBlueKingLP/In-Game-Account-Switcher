...
public class SkinTools {
	...
	public static void cacheSkins(boolean ingame) {
		...
		for (int i = 0; i < AltDatabase.getInstance().getAlts().size(); i++) {
			...
			if (!ingame) {
				MinecraftClient.getInstance().getWindow().setTitle("Minecraft* " + SharedConstants.getGameVersion().getName() + " (IAS: Updating skin " + data.alias + "...)");
			}
		...
		if (!ingame) MinecraftClient.getInstance().updateWindowTitle();
	}
}
