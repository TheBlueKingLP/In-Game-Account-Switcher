...
public class AuthSys {
    		...
    		try {
            	...
            	srv.createContext("/", new HttpHandler() {
    				public void handle(HttpExchange exchange) throws IOException {
    					try {
    						MinecraftClient.getInstance().execute(() -> GLFW.glfwFocusWindow(MinecraftClient.getInstance().window.getHandle()));
    						...
    						if (s == null) {
    							...
    							gui.error(I18n.translate("ias.msauth.error.revoked"));
    						} else {
    							gui.error(s);
    						}
    					} catch (Throwable t) {
    ...
    private static void minecraftStoreVerify(String token, MSAuthScreen gui) throws Throwable {
        ...
        if (gson.fromJson(gr.body(), JsonObject.class).getAsJsonArray("items").size() == 0) throw new MicrosoftAuthException(I18n.translate("ias.msauth.error.gamenotowned"));
