...
public abstract class SlotGui extends AbstractContainerEventHandler implements Widget, NarratableEntry {
	...
	protected boolean selectItem(int p_selectItem_1_, int p_selectItem_2_, double p_selectItem_3_,
	...
	protected abstract void renderBackground(PoseStack ms);

	protected void updateItemPosition(int p_updateItemPosition_1_, int p_updateItemPosition_2_,
			int p_updateItemPosition_3_, float p_updateItemPosition_4_) {
	}

	protected abstract void renderItem(PoseStack ms, int p_renderItem_1_, int p_renderItem_2_, int p_renderItem_3_,
			int p_renderItem_4_, int p_renderItem_5_, int p_renderItem_6_, float p_renderItem_7_);
	...
	public int getItemAtPosition(double p_getItemAtPosition_1_, double p_getItemAtPosition_3_) {
		...
		return p_getItemAtPosition_1_ < (double) this.getScrollbarPosition() && p_getItemAtPosition_1_ >= (double) i
				&& p_getItemAtPosition_1_ <= (double) j && l >= 0 && k >= 0 && l < this.getItemCount() ? l : -1;
	}
	...
	public boolean isMouseInList(double p_isMouseInList_1_, double p_isMouseInList_3_) {
		return p_isMouseInList_3_ >= (double) this.y0 && p_isMouseInList_3_ <= (double) this.y1
				&& p_isMouseInList_1_ >= (double) this.x0 && p_isMouseInList_1_ <= (double) this.x1;
	}
	...
	public void render(PoseStack ms, int p_render_1_, int p_render_2_, float p_render_3_) {
		if (this.visible) {
			...
			RenderSystem.setShaderTexture(0, Gui.BACKGROUND_LOCATION);
			...
			bufferbuilder.vertex((double) this.x0, (double) (this.y0 + 4), 0.0D).uv(0.0F, 1.0F).color(0, 0, 0, 0)
					.endVertex();
			bufferbuilder.vertex((double) this.x1, (double) (this.y0 + 4), 0.0D).uv(1.0F, 1.0F).color(0, 0, 0, 0)
					.endVertex();
			...
			bufferbuilder.vertex((double) this.x1, (double) this.y1, 0.0D).uv(1.0F, 1.0F).color(0, 0, 0, 255)
					.endVertex();
			bufferbuilder.vertex((double) this.x1, (double) (this.y1 - 4), 0.0D).uv(1.0F, 0.0F).color(0, 0, 0, 0)
					.endVertex();
			bufferbuilder.vertex((double) this.x0, (double) (this.y1 - 4), 0.0D).uv(0.0F, 0.0F).color(0, 0, 0, 0)
					.endVertex();
			tessellator.end();
			...
			if (j1 > 0) {
				...
				tessellator.end();
				bufferbuilder.begin(Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
				bufferbuilder.vertex((double) i, (double) (l1 + k1), 0.0D).uv(0.0F, 1.0F).color(128, 128, 128, 255)
						.endVertex();
				bufferbuilder.vertex((double) j, (double) (l1 + k1), 0.0D).uv(1.0F, 1.0F).color(128, 128, 128, 255)
						.endVertex();
				bufferbuilder.vertex((double) j, (double) l1, 0.0D).uv(1.0F, 0.0F).color(128, 128, 128, 255)
						.endVertex();
				bufferbuilder.vertex((double) i, (double) l1, 0.0D).uv(0.0F, 0.0F).color(128, 128, 128, 255)
						.endVertex();
				tessellator.end();
				bufferbuilder.begin(Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
				bufferbuilder.vertex((double) i, (double) (l1 + k1 - 1), 0.0D).uv(0.0F, 1.0F).color(192, 192, 192, 255)
						.endVertex();
				bufferbuilder.vertex((double) (j - 1), (double) (l1 + k1 - 1), 0.0D).uv(1.0F, 1.0F)
						.color(192, 192, 192, 255).endVertex();
				bufferbuilder.vertex((double) (j - 1), (double) l1, 0.0D).uv(1.0F, 0.0F).color(192, 192, 192, 255)
						.endVertex();
				bufferbuilder.vertex((double) i, (double) l1, 0.0D).uv(0.0F, 0.0F).color(192, 192, 192, 255)
						.endVertex();
				tessellator.end();
			}
	...
	protected void updateScrollingState(double p_updateScrollingState_1_, double p_updateScrollingState_3_,
	...
	public boolean mouseDragged(double p_mouseDragged_1_, double p_mouseDragged_3_, int p_mouseDragged_5_,
		...
		if (super.mouseDragged(p_mouseDragged_1_, p_mouseDragged_3_, p_mouseDragged_5_, p_mouseDragged_6_,
				p_mouseDragged_8_)) {
			return true;
	...
	protected void renderList(PoseStack ms, int p_renderList_1_, int p_renderList_2_, int p_renderList_3_,
			int p_renderList_4_, float p_renderList_5_) {
	...
	protected void renderHoleBackground(int p_renderHoleBackground_1_, int p_renderHoleBackground_2_,
		...
		bufferbuilder.vertex((double) this.x0, (double) p_renderHoleBackground_2_, 0.0D)
				.uv(0.0F, (float) p_renderHoleBackground_2_ / 32.0F).color(64, 64, 64, p_renderHoleBackground_4_)
				.endVertex();
		bufferbuilder.vertex((double) (this.x0 + this.width), (double) p_renderHoleBackground_2_, 0.0D)
				.uv((float) this.width / 32.0F, (float) p_renderHoleBackground_2_ / 32.0F)
				.color(64, 64, 64, p_renderHoleBackground_4_).endVertex();
		bufferbuilder.vertex((double) (this.x0 + this.width), (double) p_renderHoleBackground_1_, 0.0D)
				.uv((float) this.width / 32.0F, (float) p_renderHoleBackground_1_ / 32.0F)
				.color(64, 64, 64, p_renderHoleBackground_3_).endVertex();
		bufferbuilder.vertex((double) this.x0, (double) p_renderHoleBackground_1_, 0.0D)
				.uv(0.0F, (float) p_renderHoleBackground_1_ / 32.0F).color(64, 64, 64, p_renderHoleBackground_3_)
				.endVertex();
		tessellator.end();
	}
	...
	protected void drawContainerBackground(Tesselator tessellator) {
		...
		buffer.vertex((double) this.x0, (double) this.y1, 0.0D).uv(this.x0 / scale, (this.y1 + (int) this.yo) / scale)
				.color(32, 32, 32, 255).endVertex();
		buffer.vertex((double) this.x1, (double) this.y1, 0.0D).uv(this.x1 / scale, (this.y1 + (int) this.yo) / scale)
				.color(32, 32, 32, 255).endVertex();
		buffer.vertex((double) this.x1, (double) this.y0, 0.0D).uv(this.x1 / scale, (this.y0 + (int) this.yo) / scale)
				.color(32, 32, 32, 255).endVertex();
		buffer.vertex((double) this.x0, (double) this.y0, 0.0D).uv(this.x0 / scale, (this.y0 + (int) this.yo) / scale)
				.color(32, 32, 32, 255).endVertex();
		tessellator.end();
	}
	...
	@Override
	public void updateNarration(NarrationElementOutput arg0) {}
}
